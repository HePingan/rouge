...
assert(css.includes('body::after') && css.includes('height: max(72px, calc(env(safe-area-inset-bottom, 0px) + 42px))') && css.includes('pointer-events: none') && css.includes('#2c4568'), 'body should draw a non-interactive brighter bottom gradient mask over the exposed system/safe-area strip');
