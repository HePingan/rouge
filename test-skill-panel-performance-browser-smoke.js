...
function startServer() {
  const server = http.createServer((req, res) => {
    try {
      ...
      const rel = decodeURIComponent(url.pathname === '/' ? '/test-skill-panel-performance-browser-smoke.html' : url.pathname);
...
async function main() {
  ...
  assert(browserBin, 'Chromium/Chrome is required for skill panel performance browser smoke');
  const server = await startServer();
  const port = server.address().port;
  const userDataDir = fs.mkdtempSync('/tmp/rouge-skill-perf-profile-');
  const browser = spawn(browserBin, [
    '--headless=new', '--no-sandbox', '--disable-gpu', '--disable-dev-shm-usage',
    '--virtual-time-budget=25000', '--dump-dom', `--user-data-dir=${userDataDir}`,
    `http://127.0.0.1:${port}/test-skill-panel-performance-browser-smoke.html`,
