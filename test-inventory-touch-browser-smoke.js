...
async function main() {
  const server = http.createServer((req, res) => {
    ...
    const rel = clean === '/' ? '/test-inventory-touch-browser-smoke.html' : clean;
  ...
  const url = `http://127.0.0.1:${port}/test-inventory-touch-browser-smoke.html`;
  try {
    ...
    console.log('inventory touch browser smoke passed');
  } finally {
    server.close();
  }
}
