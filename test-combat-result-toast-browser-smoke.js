...
function serve() {
  const server = http.createServer((req, res) => {
    ...
    const file = clean === '/' ? '/test-combat-result-toast-browser-smoke.html' : clean;
...
(async () => {
  ...
  assert(browserBin, 'Chromium/Chrome is required for combat result toast browser smoke');
  ...
  const proc = spawn(browserBin, [
    ...
    '--window-size=390,844',
    `http://127.0.0.1:${port}/test-combat-result-toast-browser-smoke.html`
  ...
  assert(out.includes('combat result toast browser smoke passed'), 'pass marker missing');
  const markerIdx = out.lastIndexOf('combat result toast browser smoke passed');
  const marker = out.slice(markerIdx).match(/combat result toast browser smoke passed[^<\n]*/)?.[0];
  console.log(marker);
})().catch(err => {
  console.error(err);
  process.exit(1);
