...
async function main() {
  const server = http.createServer((req, res) => {
    ...
    const rel = clean === '/' ? '/test-inventory-bulk-sell-browser-smoke.html' : clean;
  ...
  const url = `http://127.0.0.1:${port}/test-inventory-bulk-sell-browser-smoke.html`;
  try {
    ...
    console.log('inventory bulk sell browser smoke passed');
  } finally {
    server.close();
  }
}
