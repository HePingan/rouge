...
function getAscensionRequirementRealmName() {
  return typeof REALMS !== 'undefined' && REALMS?.[ASCENSION_REQUIREMENTS.realmIndex]
    ? REALMS[ASCENSION_REQUIREMENTS.realmIndex].name
    : '渡劫圆满';
}
function getAscensionStatus(player, materials = (typeof playerMaterials !== 'undefined' ? playerMaterials : {})) {
  ...
  if ((player.realmIndex || 0) < ASCENSION_REQUIREMENTS.realmIndex) missing.push(`境界需达到${getAscensionRequirementRealmName()}`);
...
function getImmortalBodyActionState(player, materials = (typeof playerMaterials !== 'undefined' ? playerMaterials : {})) {
  const state = normalizeAscensionState(player?.ascension);
  const current = IMMORTAL_BODY_TIERS[state.immortalBody.level] || IMMORTAL_BODY_TIERS[0];
  const next = IMMORTAL_BODY_TIERS[state.immortalBody.level + 1] || null;
  const owned = Math.max(0, Number(materials?.immortal_marrow || 0));
  const need = next ? Number(next.cost || 0) : 0;
  if (!player) return { canUpgrade: false, reason: '角色未初始化', current, next, owned, need, code: 'no_player' };
  if (!state.ascended) return { canUpgrade: false, reason: '飞升后可淬炼仙躯', current, next, owned, need, code: 'not_ascended' };
  if (!next) return { canUpgrade: false, reason: '仙躯已满', current, next, owned, need, code: 'max' };
  if (owned < need) return { canUpgrade: false, reason: `需要仙髓（${owned}/${need}）`, current, next, owned, need, code: 'materials' };
  return { canUpgrade: true, reason: '可淬炼仙躯', current, next, owned, need, code: 'ready' };
}
function upgradeImmortalBody(player, materials = (typeof playerMaterials !== 'undefined' ? playerMaterials : {})) { const actionState = getImmortalBodyActionState(player, materials); if (!actionState.canUpgrade) return { ok: false, reason: actionState.reason, code: actionState.code, actionState }; player.ascension = normalizeAscensionState(player.ascension); materials.immortal_marrow = Math.max(0, Number(materials.immortal_marrow || 0) - actionState.need); player.ascension.immortalBody.level = actionState.next.level; if (typeof player.recalcStats === 'function') player.recalcStats(); return { ok: true, tier: actionState.next, actionState }; }
...
function getLawUpgradeActionState(player, lawId, materials = (typeof playerMaterials !== 'undefined' ? playerMaterials : {})) {
  const law = LAW_DEFINITIONS[lawId];
  const state = normalizeAscensionState(player?.ascension);
  const level = law ? Number(state.laws[lawId] || 0) : 0;
  const cost = getLawUpgradeCost(level);
  const owned = law ? Math.max(0, Number(materials?.[law.materialId] || 0)) : 0;
  const matName = law ? getMaterialDisplayName(law.materialId) : '材料';
  if (!player) return { canUpgrade: false, reason: '角色未初始化', code: 'no_player', law, level, cost, owned };
  if (!law) return { canUpgrade: false, reason: '未知法则', code: 'unknown', law, level, cost, owned };
  if (!state.ascended) return { canUpgrade: false, reason: '飞升后可参悟法则', code: 'not_ascended', law, level, cost, owned };
  if (level >= 10) return { canUpgrade: false, reason: '法则已圆满', code: 'max', law, level, cost, owned };
  if (owned < cost) return { canUpgrade: false, reason: `需要${matName}（${owned}/${cost}）`, code: 'materials', missing: law.materialId, law, level, cost, owned };
  return { canUpgrade: true, reason: '可参悟法则', code: 'ready', law, level, cost, owned };
}
function upgradeLaw(player, lawId, materials = (typeof playerMaterials !== 'undefined' ? playerMaterials : {})) { const actionState = getLawUpgradeActionState(player, lawId, materials); if (!actionState.canUpgrade) return { ok: false, reason: actionState.reason, code: actionState.code, missing: actionState.missing, actionState }; player.ascension = normalizeAscensionState(player.ascension); materials[actionState.law.materialId] = Math.max(0, Number(materials[actionState.law.materialId] || 0) - actionState.cost); player.ascension.laws[lawId] = actionState.level + 1; if (typeof player.recalcStats === 'function') player.recalcStats(); return { ok: true, law: actionState.law, level: actionState.level + 1, actionState }; }
...
function getAscensionClassSkillActionState(player, nodeId, materials = (typeof playerMaterials !== 'undefined' ? playerMaterials : {})) {
  const state = normalizeAscensionState(player?.ascension);
  const cls = state.classId;
  const node = cls ? (ASCENSION_CLASS_TREES[cls]?.nodes || []).find(n => n.id === nodeId) : null;
  const owned = Math.max(0, Number(materials?.immortal_jade_ascended || 0));
  const need = node ? Number(node.cost || 0) : 0;
  if (!player) return { canLearn: false, reason: '角色未初始化', code: 'no_player', node, owned, need };
  if (!state.ascended) return { canLearn: false, reason: '飞升后可学习仙职技能', code: 'not_ascended', node, owned, need };
  if (!cls) return { canLearn: false, reason: '请先选择仙职', code: 'no_class', node, owned, need };
  if (!node) return { canLearn: false, reason: '该仙职无此节点', code: 'unknown', node, owned, need };
  if (state.classSkills[nodeId]) return { canLearn: false, reason: '节点已习得', code: 'learned', node, owned, need };
  if (owned < need) return { canLearn: false, reason: `需要仙玉（${owned}/${need}）`, code: 'materials', missing: 'immortal_jade_ascended', node, owned, need };
  return { canLearn: true, reason: '可学习', code: 'ready', node, owned, need };
}
function upgradeAscensionClassSkill(player, nodeId, materials = {}) { const actionState = getAscensionClassSkillActionState(player, nodeId, materials); if (!actionState.canLearn) return { ok: false, reason: actionState.reason, code: actionState.code, missing: actionState.missing, actionState }; player.ascension = normalizeAscensionState(player.ascension); materials.immortal_jade_ascended = Math.max(0, Number(materials.immortal_jade_ascended || 0) - actionState.need); player.ascension.classSkills[nodeId] = 1; if (typeof player.recalcStats === 'function') player.recalcStats(); return { ok: true, node: actionState.node, actionState }; }
...
function completeDemonWarRun(player, materials = {}, options = {}) { player.ascension = normalizeAscensionState(player?.ascension); const actionState = getDemonWarActionState(player, materials); if (!actionState.active) return { ok: false, reason: actionState.settleReason || '未在仙魔战场' }; const total = actionState.total; const score = actionState.progress; if (!actionState.canSettle) return { ok: false, reason: actionState.settleReason || `尚未打通仙魔战场（${score}/${total}）`, score, total }; const rng = typeof options.rng === 'function' ? options.rng : Math.random; const wasFirstClear = Number(player.ascension.demonWar.clears || 0) <= 0; const bannerReward = wasFirstClear ? 1 : (rng() < Math.min(0.75, 0.35 + score * 0.08) ? 1 : 0); player.ascension.demonWar.active = false; player.ascension.demonWar.clears += 1; player.ascension.demonWar.bestClear = Math.max(player.ascension.demonWar.bestClear, score); materials.immortal_jade_ascended = (materials.immortal_jade_ascended || 0) + 8 + score * 2; materials.law_fragment_void = (materials.law_fragment_void || 0) + 2; materials.law_fragment_nether = (materials.law_fragment_nether || 0) + 2; if (bannerReward > 0) materials.demon_war_banner = Math.max(0, Number(materials.demon_war_banner || 0)) + bannerReward; const rewards = { immortal_jade_ascended: 8 + score * 2, law_fragment_void: 2, law_fragment_nether: 2 }; if (bannerReward > 0) rewards.demon_war_banner = bannerReward; return { ok: true, score, rewards }; }
