...
async function main() {
  const server = http.createServer((req, res) => {
    ...
    const rel = clean === '/' ? '/test-inventory-bulk-decompose-browser-smoke.html' : clean;
  ...
  const url = `http://127.0.0.1:${port}/test-inventory-bulk-decompose-browser-smoke.html`;
  try {
    ...
    console.log('inventory bulk decompose browser smoke passed');
  } finally {
    server.close();
  }
}
