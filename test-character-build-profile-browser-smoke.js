...
function startServer() {
  const server = http.createServer((req, res) => {
    try {
      ...
      const rel = decodeURIComponent(url.pathname === '/' ? '/test-character-build-profile-browser-smoke.html' : url.pathname);
...
async function main() {
  ...
  assert(browserBin, 'Chromium/Chrome is required for character build browser smoke');
  const server = await startServer();
  const port = server.address().port;
  const userDataDir = fs.mkdtempSync('/tmp/rouge-char-build-smoke-profile-');
  const browser = spawn(browserBin, [
    '--headless=new', '--no-sandbox', '--disable-gpu', '--disable-dev-shm-usage',
    '--virtual-time-budget=15000', '--dump-dom', `--user-data-dir=${userDataDir}`,
    `http://127.0.0.1:${port}/test-character-build-profile-browser-smoke.html`,
