...
const claimChapterStart = main.indexOf('function claimChapterBonus(chapterId)');
const claimChapterMark = main.indexOf('player.stageProgress.chapterBonusClaimed[chapterId] = true;', claimChapterStart);
const claimChapterReward = main.indexOf('applyConfiguredRewards(chapter.chapterBonus)', claimChapterStart);
assert(claimChapterStart >= 0 && claimChapterMark > claimChapterStart && claimChapterReward > claimChapterMark, 'chapter bonus should mark claimed before reward side effects to avoid duplicate rewards on interruption');
const stageCompleteStart = main.indexOf('window.onStageComplete = function onStageComplete()');
const firstClearMark = main.indexOf('progress.firstClearClaimed[stageId] = true;', stageCompleteStart);
const firstClearReward = main.indexOf('firstGain = applyConfiguredRewards(stage.firstClearRewards || {})', stageCompleteStart);
assert(stageCompleteStart >= 0 && firstClearMark > stageCompleteStart && firstClearReward > firstClearMark, 'stage first-clear should mark claimed before first-clear reward side effects');
...
assert(linkedTokens.every(token => token === '20260607safearea11'), 'all linked assets should use current stage enter cachebuster');
