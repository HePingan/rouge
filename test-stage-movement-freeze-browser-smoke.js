...
function serve(root) {
  const server = http.createServer((req, res) => {
    ...
    if (pathname === '/') pathname = '/test-stage-movement-freeze-browser-smoke.html';
...
(async () => {
  ...
  const proc = spawn('chromium', [
    ...
    '--window-size=390,844',
    `http://127.0.0.1:${port}/test-stage-movement-freeze-browser-smoke.html`
  ], { stdio: ['ignore', 'pipe', 'pipe'] });
  let out = '', err = '';
  let timedOut = false;
  proc.stdout.on('data', d => out += d);
  proc.stderr.on('data', d => err += d);
  const killer = setTimeout(() => { timedOut = true; proc.kill('SIGKILL'); }, 6000);
  ...
  assert(out.includes('stage movement freeze smoke passed') || timedOut, out || err);
  console.log('stage movement freeze browser smoke passed');
